    ...
    public class UsersController : ControllerBase
    {
        ...
        public async Task<ActionResult> UserPUT([FromBody] DTOChangeUserData userData)
        {
            ...
            try
            {
                ...
                var userIdNumero= User.FindFirstValue(ClaimTypes.NameIdentifier);
                if (int.TryParse(userId, out int userIdNumero1))
                {
                    // Resto del código para actualizar los datos del usuario usando userId...
                    //Se busca en base de datos si el usuario existe en base a su id

                    var usuarioUpdate = await _context.Usuarios.AsTracking().FirstOrDefaultAsync(x => x.Id == userIdNumero1);
